...
Out.wcv_ave_coarse=zeros(n,3);
Out.wcv_var_coarse=zeros(n,3);
Out.wcv_coarse_angle_mean=zeros(n,1); 
Out.wcv_coarse_angle_var=zeros(n,1);
Out.wcv_coarse_norm_mean=zeros(n,1);
Out.wcv_coarse_norm_var=zeros(n,1);

Out.wcv_med_coarse=zeros(n,3);
Out.wcv_coarse_med_angle_mean=zeros(n,1);
Out.wcv_coarse_med_angle_var=zeros(n,1);
Out.wcv_coarse_med_norm_mean=zeros(n,1);
Out.wcv_coarse_med_norm_var=zeros(n,1);
% pca_wcv=zeros(n,3);
% pca_wcv_evr=zeros(n,1);
for i=1:n
    Out.wcv_ave_coarse(i,:)=mean(fw(vring{i},:)); %each vertex in the coarse mesh has an vector which is average of the wcv of the one ring neighbors
    Out.wcv_med_coarse(i,:)=median(fw(vring{i},:)); 
    Out.wcv_var_coarse(i,:)= var([fw(vring{i},:);fw(i,:)]);
    wcv_coarse_angle=vectorAngle3d(Out.wcv_ave_coarse(i,:),fw(vring{i},:));% the angles between the averg wcv and its 1 ring wcv
    wcv_coarse_angle(isnan(wcv_coarse_angle))=eps; 
    Out.wcv_coarse_angle_mean(i,:)=mean(wcv_coarse_angle);
    Out.wcv_coarse_angle_var(i,:)=var(wcv_coarse_angle);
    
    wcv_coarse_med_angle=vectorAngle3d(Out.wcv_med_coarse(i,:),fw(vring{i},:));
    wcv_coarse_med_angle(isnan(wcv_coarse_med_angle))=eps;
    Out.wcv_coarse_med_angle_mean(i,:)=mean(wcv_coarse_med_angle);
    Out.wcv_coarse_med_angle_var(i,:)=var(wcv_coarse_med_angle);
%     wcv_angle_coarse(isnan(wcv_angle_coarse))=eps;
    wcv_coarse_norm=abs(vectorNorm3d(Out.wcv_ave_coarse(i,:))-vectorNorm3d(fw(vring{i},:))); % the difference between the norm of the averaged wcv and the norm of its 1 ring wcv
    Out.wcv_coarse_norm_mean(i,:)=mean(wcv_coarse_norm);
    Out.wcv_coarse_norm_var(i,:)=var(wcv_coarse_norm);
    
    wcv_coarse_med_norm=abs(vectorNorm3d(Out.wcv_med_coarse(i,:))-vectorNorm3d(fw(vring{i},:)));
    Out.wcv_coarse_med_norm_mean(i,:)=mean(wcv_coarse_med_norm);
    Out.wcv_coarse_med_norm_var(i,:)=var(wcv_coarse_med_norm);
